...
import { scanLeaderboard, type LeaderboardPeriod } from "@/lib/db/public";
import { formatCount, powerLabel, vehicleTitle } from "@/lib/utils";
import { cn } from "@/lib/utils";

export const metadata: Metadata = {
  title: "Scan leaderboard",
  description: "The most scanned builds on BuildTag: all time, this month, this week and today.",
};
...
export default async function LeaderboardPage({ searchParams }: PageProps<"/leaderboard">) {
  ...
  const rows = await scanLeaderboard(period, 50);
  const current = PERIODS.find((p) => p.id === period)!;
  const top = rows[0]?.scans ?? 0;
  ...
  return (
    <div className="mx-auto max-w-[1720px] px-4 py-10 sm:px-6 lg:px-10 2xl:px-16 md:py-14">
      ...
      <p className="mt-3 max-w-xl text-foreground/80">Every decal scan counts. Park somewhere busy, get scanned, climb the board. Public builds only.</p>

      <nav aria-label="Leaderboard period" className="mt-8 flex flex-wrap gap-2">
        {PERIODS.map((p) => (
          <Link
            key={p.id}
      ...
      {rows.length === 0 ? (
        <div className="panel mt-8 p-10 text-center">
          <p className="text-2xl">No scans yet {period === "all" ? "" : current.label.toLowerCase()}.</p>
          <p className="mt-2 text-sm text-muted-foreground">Be the first. Print your tag, stick it on, get scanned.</p>
          <Link href="/signup" className="btn-signal mt-6">
            Create your build
          </Link>
        </div>
      ) : (
            ...
            return (
              <li key={b.slug} className={cn("relative bg-surface", rank <= 3 && "bg-[linear-gradient(90deg,rgba(255,45,122,0.10),transparent_55%)]")}>
                <Link href={`/build/${b.slug}`} className="flex items-center gap-3 px-3 py-3 transition-colors hover:bg-white/5 sm:gap-5 sm:px-5">
                  <span
                    className={cn(
                      "w-10 shrink-0 text-center font-display text-2xl font-extrabold tabular-nums sm:text-3xl",
                      rank === 1 ? "text-neon-amber" : rank === 2 ? "text-foreground" : rank === 3 ? "text-[#d08a4a]" : "text-muted-foreground",
                    )}
                  >
                    {rank}
                  </span>
