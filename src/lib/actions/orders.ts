...
/** Places an order from an approved production snapshot (prices come from the database). */
export async function placeOrderAction(_prev: ActionResult | null, form: FormData): Promise<ActionResult> {
  ...
  if (!parsedShip.success || !parsedPlace.success) {
    ...
    return { ok: false, error: "Check the highlighted fields.", fieldErrors: fe };
  }
...
export async function startCheckoutAction(orderId: string): Promise<ActionResult<{ url: string } | null>> {
  ...
  if (o.status !== "awaiting_payment") return { ok: false, error: "This order is not awaiting payment." };
  ...
  await client.from("orders").update({ payment_provider: provider.key, payment_reference: session.reference, payment_status: "pending" }).eq("id", o.id);
...
export async function cancelOrderAction(orderId: string): Promise<ActionResult> {
  ...
  if (!data) return { ok: false, error: "Only unpaid orders can be cancelled." };
  await client.from("order_events").insert({ order_id: parsed.data, status: "cancelled", note: "Cancelled by customer", actor: "customer" });
  revalidatePath("/dashboard/orders");
...
const adminStatusSchema = z.object({
  ...
  status: z.enum(["draft", "awaiting_payment", "paid", "preparing_artwork", "submitted_to_printer", "in_production", "shipped", "delivered", "cancelled", "production_error"]),
