...
`supabase/seed/demo_ghost_supra.sql` seeds the **GHOST 2022 Toyota GR Supra** (24 modifications, vehicle socials, 30 days of scans, product and social clicks, 1,284 likes) under a demo user (`demo@buildtag.example`, random unknown password). Social URLs are obvious placeholders. Images are generated by `pnpm demo:images` into `public/demo/` and referenced with the `demo/` storage-path prefix.
